...
void Game::handleEvents() {
	...
	if (w_pressed && !s_pressed)
		if (player.top_hit) player.yvel = 0;
			...
			if (player.bottom_hit && !player.is_dead) {
		...
		for (int i = 0; i < 27*2-1; i++) {
			...
			if (strcmp(c.c_str(), " ") != 0) {
				...
				case 4:
					...
					tile->setHitbox(tile->getDest().x + 15, tile->getDest().y + 34, tile->getDest().w-26, 20);
					tile->isSolid = true;
					tile->isTrap = true;
					break;
				case 5:
					...
					break;
				case 6:
					tile->setTexture(tiles.at(6).c_str(), renderer);
					tile->setHitbox(tile->getDest().x+5*3, tile->getDest().y+4*3, tile->getDest().w-10*3, tile->getDest().h-8*3);
					tile->isCoin = true;
					break;
				default:
					break;
	...
	for (int i = 0; i < 15; i++) {
		for (int j = 0; j < 27; j++) {
			if (player.collision(map[i][j])) {
				...
				else if (map[i][j].isCoin) {
					SDL_SetTextureAlphaMod(map[i][j].getTexture(), 0);
				}
			}
		}
	}
